		...
		function make(){
			...
			var new_items = function(){
				...
				$('option', this).each(function(){
					...
					if ($option.data('alias')) {
						$item.addClass($option.data('alias'))
					}
			...
			$select.bind('jselector:update', function(){
				...
				if(!$selected_option.filter('[selected]').length && placeholder){
					...
					$span.removeAttr('class').addClass($options.filter(':selected').data('alias')).html($selected_item.html());
				}
			...
			$select.change(function(){
				...
				$span.html($selected_item.html()).removeAttr('class').addClass($options.filter(':selected').data('alias')).removeClass('placeholder');
			});
